...
class TestLoadCIFAR10:
    ...
    def test_load_cifar10(self, cifar10_data):
        ...
        train_loader, val_loader, test_loader = cifar10_data
        ...
        assert isinstance(val_loader, torch.utils.data.DataLoader), "Expected val_loader to be a DataLoader instance"
        ...
        val_data, val_labels = next(val_data_iter)
        test_data, test_labels = next(test_data_iter)

        # Validate the number of images in the train, val, and test sets
        ...
        assert total_train_images == 45000, f"Expected 45000 training images, but got {total_train_images}"
        assert total_val_images == 5000, f"Expected 5000 validation images, but got {total_val_images}"
        ...
        assert val_data.shape[1:] == (3, 32, 32), f"Expected val images of shape (3, 32, 32), but got {val_data.shape[1:]}"
        ...
        assert len(val_labels) == 64, f"Expected 64 val labels, but got {len(val_labels)}"
        ...
        logging.info(f"Val data shape: {val_data.shape}, Val labels shape: {val_labels.shape}")
